class Achievements
{
  ...
  {
    if (msql.connect())
      msql.query("UPDATE Achievements  SET collectedAchievements = '%s' FROM Achievements INNER JOIN User_has_Achievements WHERE collectedAchievements = '1', Use_idUser = '%s'", chieveFalse, user.currentUser);
